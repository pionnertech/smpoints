...
$Query_aux = "SELECT FAC_AUX_USER FROM FAC WHERE FAC_CODE = " . $_SESSION['TxtCode'];
    ...
    ?>
     ...
     <?
          ...
          <?  
                ...
                <tr>
                    <input type="hidden" value="<? printf($fila1[5]) ?>">
                    <td><span style="color: #295CD3;"><? printf($fila1[0]) ?> &ensp; <? printf($fila1[4]) ?></span></td>
                    <td><span style="color: #295CD3;"><? printf($fila1[2])?></span></td>
                    <td><span style="color: #295CD3;"></span></td>
                    <td><span style="color: #295CD3;"><? printf($fila1[1])?></span></td>
                    <td><span class="rePat2" style="cursor:pointer"><i class="fa fa-repeat rePat" style="width:49%; text-align:left; cursor:pointer"></i></span><span class="rePat3" ><i class="fa fa-close delete" style="width:50%; text-align:right; cursor:pointer"></i></span></td>
                </tr> 
          ...
          ?>
              ...
              <input type="button" value="Borrar" id="eraPer">
            </div>
         </div>
     </div>
<?
        ...
        <div class="wrap-main-custom" align="center">
         ...
         <div class="row over-cliente ">
                <div class="col-lg-12">
                    <ol class="breadcrumb">
                        <li>
                            <i class="fa fa-group "></i><a>&ensp;&ensp;&ensp;Lista de clientes ingresados</a>
                        </li>
                    </ol>
                </div>
         </div>
...
<div class="row over-cliente">
     ...
     <table class="table table-bordered table-hover table-striped">
         <thead>
            <tr>
                 ...
                 <td><strong>Link interfaz comensal</strong></td>
                 <td style="min-width:130px;"><strong>Borrar</strong></td>
            </tr>
         </thead>
  ...
  <? while ($fila8 = mysqli_fetch_row($client_side)){ ?>
            <tr>
                 ...
                 <td><span>restotech.cl/Qrmotion/restotechbeta.php?f=1&facility=<? printf($fila8[1]) ?>&name=<? printf(urlencode($fila8[2])) ?></td>
...
<div class="row over-estadisticas hidden">
   ...
   <div id="searhfield">
      ...
      <div id="outputbox">
       ...
       <thead>
       <tr>
          <td><strong>Nombre Comensal</strong></td>
          <td><strong>Mesa</strong></td>
          <td><strong>fecha y hora</strong></td>
          <td><strong>Codigo Promoción consumida</strong></td>
        </tr>
       </thead>
           <tbody>
           <?  while($tracert = mysqli_fetch_row($Res_Tracert)){ ?>
               <tr>
                   ...
                   <td><span><? printf($tracert[1]) ?></span></td>
...
{ value: '0' , data: '0' }
  ...
  $('#autocomplete').autocomplete({
    ...
    onSelect: function (suggestion) {
      ...
      $.uiTableFilter( t, suggestion.value );
    }
  });
...
var object = document.querySelectorAll('#minus tr td span i.delete');


for(i= 0; i < object.length; i++){

   if (object[i].style.color == "rgb(139, 0, 0)") { 
...
var ref = document.querySelectorAll("#minus tr td:nth-child(5) span");
...
document.querySelectorAll('.thumnail')[indice].src = document.querySelector('#prev-picture').src;
    ...
    $("form #code").val(code_pro);
        ...
        document.querySelectorAll("#promo-wave tr td:nth-child(4) span")[indice].innerHTML = score;
...
$("#visita, #visita a").on('click', function(){
       $(".Cant-des-wrap").html('<input type="text" class="inClass" name="visita" id="visita" value="" placeholder="Numeros de visitas">');
...
for(i= 0; i < object.length; i++){
   if(object[i].style.color === 'rgb(139, 0, 0)'){ 
      per += document.querySelectorAll('#minus tr input')[i].value + "|";
  } 
}
...
$.ajax({ type: "POST",
    ...
    swal("Personal Borrado con exito", "", "success");
        $("#eraPer").removeClass('btn-primary');
             setTimeout( function (){
                 for(y= 0; y < object.length; y++){
                    if(object[y].style.color === 'rgb(139, 0, 0)'){ 
                        document.querySelectorAll('#minus tr')[y].innerHTML = '';
                                } 
                              }
                            }, 1000);
                        }
...
proCode = proCode.replace(/^0+/g, '');
   ...
   $.ajax({
       ...
       success : function(data){
          console.info(data);
            swal("Tu promoción ha sido ingresada","",'success');
            $("textarea").val('');
            $("#codigo_pro").val('');
            $("#ba-score-input").val('');
...
var fechaIng     = di.getFullYear() + "-" + (di.getMonth()+1) + "-" + di.getDate() + " " + di.getHours() + ":" + di.getMinutes() + ":" + di.getSeconds();
    ...
    $.ajax({
            ...
            success: function(data){
                console.info(data);
                insertCells(nom, surn, '', fechaDisplay, 'minus', data, 'sbadm');
                    $("#GName").val('');
                        $("#GSurname").val('');
                           $("#aux-user").val('');
                                $("#aux-pass").val('');
                             ...
                             swal("Personal ingresado", "", "success");
            },
            error : function (error){
                          ...
                          $("#pass1").val('');
                            $("#pass2").val('');
                              $("#user-name-custom").val('');
                                $("#matching").html('');
                                  swal("Cliente Ingresado!", "", "success");
            },
            error : function (error){
                alert("No se puedo conectar con el servidor!");
            }
   ...
   time = typeof time !== 'undefined' ? time : 0;
    ...
    if(type == 'visita' && time === 0){
        $("#showPre p:first").html(value);
           swal("Variable Visita Ingresada!", "", "success");
    }  else if (type === 'compra'){
          $("#showPre p:last-child").html(value);
               swal("Monto por boleta Ingresado!", "", "success");
    } else {
